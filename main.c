...
int main () {
    ...
    for (int i = 0;i<length;i++) {
        if (checkValidity(array[i], n)) {
            list[count] = infix_to_postfix(array[i]);
            count++;
        }
    }
    int i;
    ...
    for (;;) {
        ...
        switch (i) {
            ...
            case 2 :
                convertToInfix(list, count);
                break;

            case 3 :
                evaluatePrefix1(list, count);
                break;
...
int checkValidity(char *string, char *reason ) {
    ...
    for (;*(q+1) ;q++) { //searching the string
        ...
        else if (isOperator(*q) && !(isdigit(*(q+1)) || contains(*(q+1), "([{<") || (*(q+1) == '-') )) {
            handleReason(string, q, reason);
            return 0;
        }
        else if (isOperator(*q) && (isOperator(*(q+1))) && (*(q+1) != '-')) {
            sprintf(string, "2 operators %c and %c cannot be beside eachother!");
            return 0;
        }
        else if (isOpening(*q) && !(isOpening(*(q+1)) || isdigit(*(q+1)) || (*(q+1) == '-'))) {
...
int pop(Stack* s) {
    ...
    return s->array[s->size]; //this returns the element just popped!
}

void push(Stack* s, int data) {

    if (isFull_stack(s)) {
        printf("Stack is full. Doubling capacity...\n");
        doubleCapacity(s);
    }
...
void doubleCapacity(Stack* s) {
    ...
    int *s_new = realloc(s->array, sizeof(int) * (s->capacity) * 2);

    if (s_new == NULL) {

        printf("doubling failed!\n");
        free(s);
        exit(1);
    }
    else if (s_new != s->array){
        s->array = s_new;
    }
    s->capacity *= 2;
    s_new = NULL;
